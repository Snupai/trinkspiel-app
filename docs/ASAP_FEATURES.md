...
- Sign the release AAB with real release credentials.
  - Status: local default keystore file exists, but `scripts/check-release-signing-config.sh` reports missing store password, key alias, and key password. `scripts/prepare-release-signing-handoff.sh` now writes a non-secret handoff folder with current checker output, environment/properties templates, and signing steps for the trusted machine that owns the real credentials.
  - Evidence needed: `scripts/check-release-signing-config.sh` reports `READY` with private local signing-file permissions, a private-key alias, a successful temporary signing probe, and no Android debug signing material; `scripts/verify-release-ready.sh` reports the release bundle is current, signed with non-debug signing material, and verified; the generated package includes `release-signing-handoff/` for credential-owner handoff; and `scripts/finalize-play-store-release.sh` exits upload-ready.
- Host the privacy policy.
  - Status: `docs/privacy-policy.html` exists, `scripts/check-privacy-policy-url.sh --local-only` validates the local file, and `scripts/prepare-privacy-policy-hosting.sh` now writes an exact-match static hosting bundle with `privacy-policy.html`, `index.html`, checksums, and hosting instructions. No hosted URL is configured.
  - Evidence needed: `SEEMOPS_PRIVACY_POLICY_URL` is set to the final public HTTPS URL, not a localhost/private/example/test URL, `scripts/check-privacy-policy-url.sh` reports the hosted page is reachable with the expected Seemops privacy-policy commitments and exactly matches `docs/privacy-policy.html`, the generated hosting bundle checksum matches the hosted file source, and `scripts/verify-release-ready.sh` passes.
